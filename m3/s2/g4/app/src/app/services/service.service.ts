...
export class ServiceService {
  ...
  getPostById(id: number): IPost {
    const ind = this.arrPosts.findIndex(post => post.id === id)
    return this.arrPosts[ind]
  }

  getActivePosts() {
    return this.arrPosts.filter(p => p.active)
  }
  ...
  updateStatus(id: number) {
    const ind = this.arrPosts.findIndex(p => p.id === id)
    this.arrPosts[ind].active = !this.arrPosts[ind].active
  }
